...
##
## scale_indices -- defines the indices of the full length vector w 
##                  that correspond to the wavelet scales.
##
##             A -- (n_w x n_a) matrix that defines pooling within 
##                  the wavelet vector (and n_a varies depending on
##                  the amount of pooling desired).
##
...
def extract_A_or_B_or_X(design_matrices, \
                        which_matrix, \
                        scale_sizes, \
                        scale_indices, \
                        break_vector, \
                        n_w):    
    if which_matrix in design_matrices:
        if sparse.issparse(design_matrices[which_matrix]):
            # User-supplied sparse matrix, check dims:
            ret = design_matrices[which_matrix]
            if len(ret.shape) != 2:
                raise Exception, "Error: Invalid dimensions for '" + str(which_matrix) + "'"
            if (ret.shape[0] != n_w):
                raise Exception, "Error: '" + str(which_matrix) + "' does not have 'n_w' rows"
        else:
            # User-must supply valid keyword:
            if (which_matrix == "A") or (which_matrix == "B"):
                if (design_matrices[which_matrix] == "within-levels"):
                    ret = within_levels_matrix(scale_sizes,scale_indices)
                elif (design_matrices[which_matrix] == "across-levels"):
                    raise Exception, "Not yet implemented..."
                else:
                    raise Exception, "'" + str(which_matrix) + "' must be one of 'within-levels', '...' or '...'"
            elif (which_matrix == 'X'):
                if (design_matrices[which_matrix] == "breaks"):
                    ret = breaks_design_matrix(break_vector,n_w)
                else:
                    raise Exception, "'" + str(which_matrix) + "' must be one of 'breaks' or '...'"
            else: 
                raise Exception, "'which_matrix' must one of 'A', 'B' or 'X'" 
            # End possible ways to specify A/B ...
    else:
        raise Exception, "Error: 'design_matrices' does not contain key '" + str(which_matrix) + "'"
    # Return the design matrix!
    return ret
    
def within_levels_matrix(scale_sizes,scale_indices,dtype="float32"):
    n_w = sum(scale_sizes)
    n_a = len(scale_indices)
    A = sparse.lil_matrix((n_w,n_a),dtype=dtype)
    for i in range(0,n_a):
        A[scale_indices[i],i] = 1.0
    return A

def breaks_design_matrix(break_vector,n_w,dtype="float32"):
    n_b = len(break_vector)
    X = sparse.lil_matrix((n_w,n_b),dtype=dtype)
    first_ix = 0
    for i in range(0,n_b):
        # Put 1's in appropriate places:
        X[first_ix:min(break_vector[i],n_w-1),i] = 1.0
        first_ix = break_vector[i]
    return X
    
def check_design_matrices(design_matrices,scale_sizes,scale_indices,break_vector,n_w):

    # The key part, setting up A, B and X:
    A = extract_A_or_B_or_X(design_matrices,'A',scale_sizes,scale_indices,break_vector,n_w)            
    B = extract_A_or_B_or_X(design_matrices,'B',scale_sizes,scale_indices,break_vector,n_w)            
    X = extract_A_or_B_or_X(design_matrices,'X',scale_sizes,scale_indices,break_vector,n_w)       
...
def to_sparse_colvec(x):
    return x
    #return sparse.csc_matrix(x).transpose()

def sample_w(A,d,B,sigma2_w,verbose=True):
    # extract length:
    n_w = A.shape[0]
    # compute mean:
    w_mean = (A*d)
    ...
    w_sd = np.sqrt(B*sigma2_w)
    # sample w:
    w_vector = rnorm(mu=w_mean,sigma=w_sd,size=n_w)
    # return:
    return w_vector
...
def sample(n, priors, \
    ...
    # Inputs:
    #
    # n :: Length of series to be sampled
    #
    ...
    #
    # mode :: How to handle non-power-of-two lengths
    # 
    # design_matrices :: dict containing entries for 'A', 'B' and 'X.
    #            For each, the corresponding key can be either
    #            a 'default' choice, or a full matrix. Default choices
    #            for the pooling are "within-levels", "..." etc.
    #
    # fixed_pars :: The name of any parameters to be fixed for the sampling
    # 
    # fixed_par_values :: Values of the parameters to be fixed
    #
    # var_changes ::
    # 
    # qtr_changes ::
    #
    # verbose :: Level of verbosity in the progress/output
    #
    #################################################################
    #
    ...
    if math.fabs(math.log(n,2)-int(math.log(n,2)))>1e-10:
        ...
        n_w = -1; # number of wavelet coefs may differ from n
    else:
        n_w = n
    ...
    # Error check design matrices:
    A, B, X = check_design_matrices(design_matrices,scale_sizes,scale_indices,var_changes,n_w)
    if verbose:
        print "A:"
        print A.todense()
        print "B:"
    ...
    if (priors['d']['Sigma'].shape != (n_d,n_d)):
        raise Exception, "Prior dimensions for 'd' (mu and Sigma) do not match"

    # Sample d:
    d = rmvnorm(mu=priors['d']['mu'],Sigma=priors['d']['Sigma'])[0,:]
    # Convert to sparse vector form:
    d = to_sparse_colvec(d)
    # print for user:
    if verbose:
        print "Sampled d: "
        print d
    ...
    if 'Sigma_w' in fixed_pars:
        ...
        # sparse-ify:
        sigma2_w = to_sparse_colvec(sigma2_w)
        
    if verbose:
        print "Sampled sigma^{2}_{w}: "
        print sigma2_w

    # Generalized: w_{1:R} ~ N(Ad,diag(B\sigma_{w}))
    w_vector = sample_w(A,d,B,sigma2_w)
    ...
    # Transform to f:
    ## TODO: Generalize to A (?) from scale_indices... 
    ...
    fixed = {'A': A, 'B': B, 'X': X}
    ret = {'y': y, 'theta': theta, 'lv': lv, 'fixed': fixed}
    return ret
...
for i in range(0,R):
    psi_nu_prior[i] = 2000.0
    psi_scale_prior[i] = 0.00001
...
Sigma_d_prior[0,0] = 0.01

# Mother wavelet priors:
for j in range(1,n_d):
    nu_w[j] = 10000.0
    scale_w[j] = 0.000001
    Sigma_d_prior[j,j] = 1.0/100000.0
